...
# When running with Keras/TensorFlow in a multi-threaded web server like Flask,
# it's crucial to handle the graph context for thread safety.
# We will load the model outside but handle the prediction inside a dedicated function.
# For modern TF 2.x, this is less critical, but good practice.
# We will use tf.keras.backend.clear_session() to ensure model isolation (optional but helpful)
tf.keras.backend.clear_session()

app = Flask(__name__)


# # --- Configuration Paths Logistic Regression Model ---
...
# # Newer versions
# # --- Configuration Paths Logistic Regression Model ---
# TEXT_MODEL_PATH = 'models/text_model_v2.joblib'
# VECTORIZER_PATH = 'models/vectorizer_v2.joblib'
# VISUAL_MODEL_PATH = 'models/visual_model.h5'


# --- Configuration Paths SVM Model ---
TEXT_MODEL_PATH = 'models/text_model_svm_v3.joblib'
...
try:
    ...
    # We load with compile=False as we only need inference weights
...
def analyze_visual(file_stream, file_type):
    ...
    FIXED: Ensures thread safety by using unique temp files for videos.
    """
    ...
    # Define a default image size for the CNN input
    ...
    if file_type.startswith('image/'):
        try:
            ...
            # Assuming 1 = Deepfake/Fake, 0 = Authentic/Real
            ...
            return {
                "type": "Error",
                "message": f"Could could not process image: {e}",
                "confidence": 0,
                "result_type": "Error"
            }
    
    elif file_type.startswith('video/'):
        # CRITICAL FIX: Use a unique temporary file path to prevent race conditions
        ...
        try:
            ...
            # Optimization: Analyze a few frames (e.g., 5 frames) to represent the video
            frames_to_analyze = 5 
            deepfake_scores = []
            
            # Calculate interval to skip frames evenly
            total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
            skip_interval = max(1, total_frames // frames_to_analyze)

            for i in range(frames_to_analyze):
                # Set position to a frame in the video
